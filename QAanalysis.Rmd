...
We can observe that samples cluster primarily by sample type, tumor or normal. Sample vial seems to have a stronger effect among the normal and tumor samples. We may consider discarding samples leading to an unbalanced distribution of the outcome across batches.
...
Then we examine how many genes change across conditions without adjustment. It is useful for comparing the model with and without surrogate variables, even we are using the logCPM taht is not the best approach.
```{r}
pValues <- f.pvalue(logCPM, mod, mod0)
sum(p.adjust(pValues, method = "BH") < 0.05)
```
There are 9547 differentially expressed genes with FDR < 5%.
...
The number of changing genes has increased by about 7% and the p-value distribution is more uniform now for the non-DE genes.

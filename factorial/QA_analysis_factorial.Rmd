...
se <- readRDS(file.path("results", "se_history.rds"))
```
...
names(dge)
saveRDS(dge, file.path("results", "dge_factorial.rds"))
```
...
barplot(dge$sample$lib.size[ord], las=1, ylab="Millions of reads",
                xlab="Samples", col=c("blue", "red")[(se$tobacco_smoking_history_indicator[ord] == "Current smoker") + 1])
...
```{r}
dge$samples$group <- se$tobacco_smoking_history_indicator
...
```{r}
saveRDS(se.filt, file.path("results", "se.factorial.filt.unnorm.rds"))
saveRDS(dge.filt, file.path("results", "dge.factorial.filt.unnorm.rds"))
```
...
cpmcutoff
nsamplescutoff <- min(table(se$tobacco_smoking_history_indicator))
nsamplescutoff
```
...
```{r}
saveRDS(se.filt2, file.path("results", "se.factorial.filt2.unnorm.rds"))
saveRDS(dge.filt2, file.path("results", "dge.factorial.filt2.unnorm.rds"))
```
...
```{r}
saveRDS(se.filt, file.path("results", "se.factorial.filt.rds"))
saveRDS(dge.filt, file.path("results", "dge.factorial.filt.rds"))
```
...
dge$samples$group <- se$tobacco_smoking_history_indicator
table(dge$samples$group)
dge.filt$samples$group <- se$tobacco_smoking_history_indicator
```
...
outcome <- paste(substr(colnames(se.filt), 9, 12), as.character(se.filt$tobacco_smoking_history_indicator), sep="-")
...
```{r}
mod <- model.matrix(~tobacco_smoking_history_indicator, data = colData(se.filt))
head(mod)
```
...
sampleDendrogram <- as.dendrogram(sampleClustering, hang = 0.1)
names(batch) <- colnames(se.filt)
outcome <- as.character(se.filt$tobacco_smoking_history_indicator)
names(outcome) <- colnames(se.filt)
...
```{r}
saveRDS(se.filt, file.path("results", "se.factorial.filt.batch.rds"))
saveRDS(dge.filt, file.path("results", "dge.factorial.filt.batch.rds"))
```
